...
parser.add_argument('--epochs', type=int, default=80,nargs='?', help='display an integer')
parser.add_argument('--alpha', type=float, default=0.6,nargs='?', help='display a float')
parser.add_argument('--beta', type=float, default=0.0,nargs='?', help='display a float')
parser.add_argument('--gamma', type=float, default=0.6,nargs='?', help='display a float')
...
parser.add_argument('--random_seed', type=int, default=2024,nargs='?')
...
# /opt/data/private/fish/DTCA/datasets/finetune/dualc/2015/inputCaption.pt
data_input_file = os.path.join("F:/KCI/datasets/finetune",task_name,dataset_type,"inputKLCaptionITM.pt")
# data_input_file = os.path.join("/opt/data/private/fish/DTCA/datasets/finetune",task_name,dataset_type,"input.pt")
...
if text_model_name == 'bert':
    ...
    model_path1 = "F:/KCI/models/roberta-base"
    config1 = AutoConfig.from_pretrained(model_path1)
...
if image_model_name == 'vit':
    model_path2 = "F:/KCI/models/vit-base-patch16-224-in21k"
...
# init DTCAModel
vb_model = DTCAModel(config1,config2,text_num_labels=5,text_model_name=text_model_name,image_model_name=image_model_name,alpha=alpha,beta=beta,gamma=gamma)
